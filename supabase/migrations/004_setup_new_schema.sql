-- =====================================================
-- LABSYNC PRODUCTION SCHEMA SETUP
-- =====================================================
-- Clean architecture with essential features including Google Calendar sync
-- Run this AFTER 003_teardown_old_schema.sql
-- =====================================================
...
CREATE TABLE projects (
  ...
  -- Hierarchy enforced via triggers or application layer (CHECK constraints can't use subqueries)
...
-- Function to enforce project-bucket lab matching
CREATE OR REPLACE FUNCTION enforce_project_bucket_lab()
RETURNS TRIGGER AS $$
BEGIN
  -- Ensure project's lab_id matches bucket's lab_id
  IF NEW.lab_id != (SELECT lab_id FROM buckets WHERE id = NEW.bucket_id) THEN
    RAISE EXCEPTION 'Project lab_id must match bucket lab_id';
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

-- Function to enforce task-project lab matching
CREATE OR REPLACE FUNCTION enforce_task_project_lab()
RETURNS TRIGGER AS $$
BEGIN
  -- Ensure task's lab_id matches project's lab_id
  IF NEW.lab_id != (SELECT lab_id FROM projects WHERE id = NEW.project_id) THEN
    RAISE EXCEPTION 'Task lab_id must match project lab_id';
  END IF;
  
  -- If this is a subtask, ensure it has the same project_id as parent
  IF NEW.parent_id IS NOT NULL THEN
    IF NEW.project_id != (SELECT project_id FROM tasks WHERE id = NEW.parent_id) THEN
      RAISE EXCEPTION 'Subtask must have same project_id as parent task';
    END IF;
  END IF;
...
-- Hierarchy enforcement triggers
CREATE TRIGGER enforce_project_hierarchy 
  BEFORE INSERT OR UPDATE ON projects
  FOR EACH ROW EXECUTE FUNCTION enforce_project_bucket_lab();

CREATE TRIGGER enforce_task_hierarchy
  BEFORE INSERT OR UPDATE ON tasks
  FOR EACH ROW EXECUTE FUNCTION enforce_task_project_lab();
...
CREATE UNIQUE INDEX idx_buckets_unique_name ON buckets(lab_id, name) WHERE deleted_at IS NULL;
...
CREATE UNIQUE INDEX idx_calendar_events_unique_google ON calendar_events(google_event_id, google_calendar_id) 
  WHERE google_event_id IS NOT NULL;
...
BEGIN
  ...
  RAISE NOTICE 'New schema has been successfully created.';
  RAISE NOTICE '';
  RAISE NOTICE 'Next steps:';
  RAISE NOTICE '1. Generate TypeScript types';
  RAISE NOTICE '2. Update application code if needed';
